...
class FavoriteAdapter(val clickListener: QuoteListener) : ListAdapter<DataItem, RecyclerView.ViewHolder>(QuoteDiffCallback()) {
    ...
    fun addHeaderAndSubmitList(list: List<ActivityModel>?){
        adapterScope.launch {
            val items = when(list){
    ...
    class ViewHolder private constructor(val binding: ItemBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(item: ActivityModel, clickListener: QuoteListener) {
            binding.quote = item
...
class QuoteListener(val clickListener: (quoteId: Int) -> Unit) {
    fun onClick(quote: ActivityModel) = clickListener(quote.key)
}
...
sealed class DataItem {
    ...
    data class QuoteItem(val quote: ActivityModel) : DataItem() {
        override val id = quote.key
    }
